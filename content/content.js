(() => {
  ...
  // Proton's composer is a SECTION.composer element containing a
  // footer.composer-actions and an editor iframe (NOT a direct
  // contenteditable — the editor body lives inside the iframe).
  function findComposers() {
    const candidates = [];

    // Primary: SECTION.composer with a composer-actions footer
    const byClass = document.querySelectorAll(
      'section[class*="composer"], div[class*="composer "]'
    );
    for (const el of byClass) {
      // Proton composers always have a composer-actions footer
      if (el.querySelector('footer[class*="composer-actions"]')) {
        candidates.push(el);
      }
    }

    // Fallback: any element with composer-actions inside it
    if (candidates.length === 0) {
      const actions = document.querySelectorAll(
        '[class*="composer-actions"]'
      );
      for (const footer of actions) {
        const container = footer.closest('[class*="composer"]') || footer.parentElement;
  ...
  // Proton's action bar is FOOTER.composer-actions containing Send, Attach, etc.
  function findActionBar(composer) {
    return (
      composer.querySelector('footer[class*="composer-actions"]') ||
      composer.querySelector('[class*="composer-actions"]') ||
  ...
  function injectButton(composer) {
    ...
    // Prevent Proton's LABEL (paperclip) from hijacking the click.
    // Must block mousedown/pointerdown too — LABEL activates on
    // pointerdown, which fires before our click handler.
    ...
    // Insert into the attachments button wrapper (next to the paperclip)
    // rather than appending to the footer directly (which puts us on a new line)
    const attachWrapper = actionBar.querySelector(
      '[class*="composer-attachments-button-wrapper"]'
    );
    if (attachWrapper) {
      // Insert after the LABEL (paperclip) + INPUT, before the other buttons
      const fileInput = attachWrapper.querySelector('input[type="file"]');
      if (fileInput && fileInput.nextSibling) {
        attachWrapper.insertBefore(btn, fileInput.nextSibling);
      } else {
        attachWrapper.appendChild(btn);
      }
    } else {
  ...
  // ── Attachment Injection via Proton's File Input ────────────
  //
  // Proton Mail's composer has a real <input type="file"> element
  // in the footer (INPUT.composer-attachments-button). We use the
  // same technique as the Gmail version: set .files via the native
  // setter and dispatch a 'change' event, which triggers Proton's
  // own upload handler — the exact same path as clicking the
  // paperclip and picking a file from disk.
  ...
  async function attachAssetsToCompose(compose, assetIds, options) {
    ...
    // Find Proton's file input in the composer footer
    const fileInput = compose.querySelector('input[type="file"]');
    if (!fileInput) {
      warn("no file input found in composer");
      return;
    }

    log("using file input:", fileInput.className, "with", fileObjects.length, "file(s)");

    // Use the native setter to assign files (bypasses React's read-only guard)
    ...
    log("attachment dispatched successfully");
  }
